...
```{r siblings, include = FALSE, dep = c('load')}
siblings <-
  persons_uil %>%
  get_siblings_by_at(
...
```{r siblings_n, include = FALSE, dep = 'siblings'}
siblings_n <-
  siblings %>% 
...
```{r html_siblings_n, echo = FALSE, include = TRUE, dep = 'siblings'}
html_siblings_n <-
  siblings_n %>%
...
```{r html_siblings_lax_n, echo = FALSE, include = TRUE, dep = c(-1)}
html_siblings_lax_n <-
  siblings_lax_n %>%
  create_kable_filt_at(
    config = config,
...
```{r siblings_prnk, include = FALSE, dep = 'siblings'}
siblings_prnk <-
  siblings %>%
...
```{r html_siblings_prnk, echo = FALSE, include = TRUE, dep = c(-1)}
html_siblings_prnk <-
  siblings_prnk %>%
  create_kable_filt_at(
    config = config,
...
```{r persons_n_filt, include = FALSE, dep = c('load')}
persons_n_filt <-
  persons_uil %>%
...
```{r html_persons_n_filt, echo = FALSE, include = TRUE, dep = c(-1)}
html_persons_n_filt <-
  persons_n_filt %>% 
  create_kable_filt_at(
    config = config,
...
I think I've done probably more investigation of my personal performance
throughout this series of write-up, so I'll spare the reader any more.
