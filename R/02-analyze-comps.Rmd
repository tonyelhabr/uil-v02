...
number of schools
in each level of competition (include.echo. District, Region, and State) and each 
...
competition type (include.echo. Calculator, Computer Science, etc.)
...
viz_persons_stats_bycompcomplvl_diffs <-
  ...
  theme(axis.text.x = element_blank()) +
  teplot::labs_xy_null() +
  labs(title = str_wrap("% Change in Score Distributions for Each Competition Type, By Competition Level", 60),
       caption = config$viz_footer)
viz_persons_stats_bycompcomplvl_diffs
...
lab_caption_persons_stats_bycompyear_diffs <-
  paste0(
    config$viz_footer,
    str_wrap(
      "Dashed line: Competition-specific average year-to-year difference percentage."
    ),
    100
  )
...
average competition-specific score and year. Taking the customary p-value threshold of 0.05
as the level of significance (where one may deduce that the
model is not predictive if the the p.value is greater than the threshold value), the year term
