...
number of schools
in each level of competition (i.e. District, Region, and State) and each 
conference classification level (1A, 2A, ... 6A).

```{r summ_n_bycomplvl, include = FALSE, dep = c('load')}
summ_n_bycomplvl <-
  bind_rows(
    schools_uil %>%
...
```{r viz_n_bycomplvl, echo = FALSE, include = TRUE, s = "asis", h = 7, w = 7, dep = c('load', 'summ_n_bycomplvl')}
viz_n_bycomplvl <-
  summ_n_bycomplvl %>%
...
```{r summ_n_bycomp, include = FALSE, dep = c('load')}
summ_n_bycomp <-
  bind_rows(
    schools_uil %>%
      summarise_n_bycomp() %>%
...
```{r scale_x_set1_funcs, include = FALSE}

```

```{r visualize_n_bycomp_common_at, include = FALSE, dep = c('load')}
...
```{r viz_n_bycomp, echo = FALSE, include = TRUE, s = "asis", h = 7, w = 7, dep = 'summ_n_bycomp'}
viz_n_bycomp <-
  summ_n_bycomp %>%
...
```{r summ_n_bycompcomplvl, include = FALSE, dep = c('load')}
...
```{r viz_n_bycompcomplvl, echo = FALSE, include = TRUE, s = "asis", h = 7, w = 7, dep = c('load', 'summ_n_bycompcomplvl')}
viz_n_bycompcomplvl <-
  summ_n_bycompcomplvl %>%
...
```{r summ_n_bycompcomplvlconf, include = FALSE, dep = c('load')}
...
```{r viz_n_bycompcomplvlconf, echo = FALSE, include = TRUE, s = "asis", h = 7, w = 7, dep = c('load', 'summ_n_bycompcomplvlconf')}
...
```{r persons_stats_bycomp, include = FALSE, dep = c('load')}
...
```{r viz_persons_stats_bycomp, echo = FALSE, include = TRUE, s = "asis", w = 7, h = 10, dep = c('load', 'persons_stats_bycomp')}
...
```{r persons_stats_bycompx, include = FALSE, dep = c('load')}
...
```{r viz_persons_stats_bycompx, include = FALSE, s = "hide", dep = c('load', 'persons_stats_bycompx')}
...
```{r viz_persons_stats_bycomp_grid, echo = FALSE, include = TRUE, s = "asis", h = 10, w = 7, dep = c('load', 'viz_persons_stats_bycompx')}
...
This could be indicative of a number of factors:
    ...
    - There may not be as as many unique students competing in larger conferences,
    leading to the possibility that the scores in the larger conference are more
    likely to be subject to high variance. [^larger]
...
[^larger]:
It was shown before that the number of schools across conferences is 
relatively even, so one would need to stratify the population by individuals rather than schools.
...
```{r persons_stats_bycompcomplvl_diffs, include = FALSE, dep = c('load', 'persons_stats_bycompx')}
...
```{r viz_persons_stats_bycompcomplvl_diffs, echo = FALSE, include = TRUE, s = "asis", h = 7, w = 7, dep = c('load', 'persons_stats_bycompcomplvl_diffs')}
...
```{r persons_stats_bycompyear_terms, include = FALSE, dep = c('load', 'persons_stats_bycompx')}
...
```{r viz_persons_stats_bycompyear_diffs, echo = FALSE, include = TRUE, s = "asis", h = 7, w = 7, dep = c('load', 'persons_stats_bycompyear_terms')}
viz_persons_stats_bycompyear_diffs <-
  ...
  labs(title = lab_title_stats_bycomp_prefix,
       subtitle = "By Year",
       caption = "Dashed line: Competition-specific average year-to-year difference percentage.")
viz_persons_stats_bycompyear_diffs
...
model is not predictive if the the p.value is greater than the thresshold value), the year term
...
```{r html_persons_stats_bycompyear_terms, echo = FALSE, include = TRUE, dep = c('load', 'persons_stats_bycompyear_terms')}
