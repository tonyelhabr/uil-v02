...
schools_all <- readr::read_csv(params$path_schools_scrape)
persons_all <- readr::read_csv(params$path_persons_scrape)

get_comps_valid <-
  function(params = NULL) {
    params$comps_list %>%
      tibble::enframe(name = "comp", value = "comp_name") %>%
      unnest(comp_name)
  }
...
clean_scrape_data <-
  function(data = NULL,
           ...
           comps_valid = get_comps_valid(params)) {
    ...
    # data_proc_1b %>% filter(school_city_complvlnum %in% c("Academy H S (32)"))
    # setdiff(data_proc_1b, data_proc_1a)
    ...
    # data_proc_1c2 <-
    #   data_proc_1b %>%
    #   mutate(school_city_complvlnum = str_replace(school_city_complvlnum, rgx_patt2, rgx_repl))
    #
    # setdiff(data_proc_1c, data_proc_1c2)
    # setdiff(data_proc_1c2, data_proc_1c)
    # setdiff(data_proc_1c, data_proc_1b)
    # setdiff(data_proc_1b, data_proc_1c)
    ...
    # # data_proc_2 %>% pull(city_complvlnum) %>% str_subset("^\\(")
    ...
    # setdiff(data_proc_2a, data_proc_2)
...
schools_clean <- clean_scrape_data(schools_all, params = params)
persons_clean <- clean_scrape_data(persons_all, params = params)
