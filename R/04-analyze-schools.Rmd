...
```{r schools_stats, include = FALSE, dep = c('load')}
schools_stats <-
  schools_uil %>%
  add_stats_cols_by_at(cols_grp = c("school", "city"))
```

```{r html_schools_stats_filt, echo = FALSE, include = TRUE, dep = c(-1)}
...
```{r schools_stats_byyear, include = FALSE, dep = c('load')}
...
```{r html_schools_stats_byyear, echo = FALSE, include = TRUE, dep = c(-1), eval = FALSE}
...
```{r visualize_map_tier3_at, include = FALSE, dep = c('load')}
  ...
  function(data = NULL,
    ...
    data %>%
      ...
      labs(title = paste0("Most Dominant Schools"),
           caption =
             str_wrap(
               paste0("Dominance is evaluated according to total percent rank."),
               80
             ))
  }
```
...
```{r viz_map_schools_stats_tier3_byyear, echo = FALSE, include = TRUE, s = "asis", h = 7, w = 7, dep = 'schools_stats_byyear'}
...
if(config$export_viz) {
  ...
  gganimate::gganimate(
    viz_map_schools_stats_tier3_byyear,
    filename = file.path(config$dir_viz, "viz_map_tier3_byyear.gif"),
    saver = "gif",
    # interval = 0.5,
    fps = 2,
    loop = 0
...
```{r viz_map_schools_stats_tier3, echo = FALSE, include = TRUE, s = "asis", h = 7, w = 7, dep = 'school_stats'}
...
there some kind of temporal trend for schools? My intuition says that there should _not_
...
is in many different contexts. I should not that I think these things may only be shown
to be true when also factroing in competition type--it seems more likely that 
...
Also, I think it's a good idea to "rescale" the year value
to have a first value of 1 (corresponding to the first year in the scraped data--2005), 
with subsequent years taking on subsequent integer values. (This variable is named `year_idx`).

[^previous]"
Actually, I don't specifically enforce the critiera that the previous year is used.
...
```{r schools_stats_bycomp_terms, include = FALSE, dep = c('load')}
...
```{r viz_schools_stats_bycomp_pv, echo = FALSE, include = TRUE, s = "asis", h = 7, w = 7, dep = c(-1)}
...
lab_title_schools_stats_bycomp_pv <-
  str_wrap(
    paste0(
      "Distribution of P-Values for Linear Regression Models"
  ), 60
)
...
lab_caption_schools_stats_bycomp_pv <-
  str_wrap(
    paste0(
      ...
      "Nonetheless, the even distribution of the year term indicates that year has no significant",
...
my two of my hypotheses are valid--recent performance does seem to be predictive
of school performance in a given competition type in any given year.
while year itself is not (meaning that there is no temporal trend indicating that performance
improves or worsens over time). 
However, my other thought that school itself has some kind of predictive value
does __not__ appear to be true.
[^robinson]
...
Perhaps the deduction that, in general, individual schools do _not_ tend to dominate
...
defeated at each competition level for each school should re-inforce this inference.


```{r viz_schools_n_defeat_pct, include = TRUE, s = "asis", h = 7, w = 7, dep = c('load')}
schools_n_defeat <-
  schools_uil %>% 
...
viz_schools_n_defeat_pct <-
  ...
  teplot::theme_te_facet_dx() +
  labs(
    title = "School-Specific Average % of Total Possible Competitors Defeated, by Competition Level",
...
Indeed, observing that the histograms do not show any noticeable skew to the right
supports the notion that, in general, individual schools are not dominating specific competition types.
If this notion were true, we would see some non-trivial right-hand skew.
...
This observation is not all so suprising given that if it were true that
