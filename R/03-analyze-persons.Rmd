...
advancement and state competition appearances. The heavily right skewed
distribution of values gives an indication of the difficulty of succeeding consistently.

```{r persons_stats_n_x, echo = FALSE, include = TRUE, eval = FALSE}
persons_stats %>%
  count(n_advanced_sum) %>% 
  arrange(desc(n_advanced_sum)) %>% 
  create_kable()
persons_stats %>%
  count(n_state_sum) %>% 
  arrange(desc(n_state_sum)) %>% 
  create_kable()
```
...
persons_stats_n_x <-
  bind_rows(
    ...
    mutate(lab_stat = "Compeition Advancements"),
  persons_stats %>%
    count_rename_at("n_state_sum") %>% 
    mutate(lab_stat = "State Competition Appearances")
  )
...
viz_persons_stats_n_x <-
  ...
  geom_col() +
  facet_wrap(~lab_stat, scales = "free_y") +
  ...
  teplot::labs_xy_null() +
  labs(
    title = "Distribution of Individual Advancements and State Appearances",
    y = "Log 10 of Count of Occurrence",
