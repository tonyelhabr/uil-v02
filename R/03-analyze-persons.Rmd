...
[^previous]:
See a separate write-up for this visual.

```{r viz_persons_n_top_byconf, eval = FALSE}
...
In particular, let's look at average and sum of placing percent rank (`prnk`) and of
competitors defeated (`n_defeat`). [^defeat]

[^defeat]:
Competitors defeated is defined as 
the number of scores less that that of a given individual for a unique compeition,
...
```{r html_persons_stats_filt, echo = FALSE, include = TRUE}
html_persons_stats_filt
```
